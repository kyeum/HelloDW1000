...
#define POLL 5
#define POLLfromOne 6 // 1 <-> 3 : 6, 2 <-> 3 : 7
#define POLLfromTwo 9 // 1 <-> 3 : 6, 2 <-> 3 : 7
...
void setup() {
    ...
    Serial.println(F("###UWB3 - Responder###"));
...
void loop() {
    ...
    if (receivedAck) {
        ...
        else if (msgId == POLLfromOne) {
              poll_received = true;
              Serial.println("received poll;");
              timePollReceived = DW1000Ng::getReceiveTimestamp();
              transmitRange(RANGE_To1);
              noteActivity();
        } 
        else if (msgId == POLLfromTwo) {
              poll_received = true;
              Serial.println("received poll;");
              timePollReceived = DW1000Ng::getReceiveTimestamp();
              transmitRange(RANGE_To2);
              noteActivity();
        } 
      }
}
